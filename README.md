...
## Dependencies
```
pip install numpy pandas matplotlib seaborn scikit-learn fredapi yfinance python-dotenv scipy
```

*Note: a FRED API key is required. Create a `.env` file at the project root with `FRED_API_KEY="your_key"`. A free key is available at [fred.stlouisfed.org](https://fred.stlouisfed.org/docs/api/api_key.html).*
